...
# This script processes news articles to detect bias using OpenAI's Mistral model.
...
def save_results(results, output_file='Train_mistral-summary_leaderboard.csv'):
    """ Save CSV results, including explanations. """
    results_df = pd.DataFrame(results, columns=['unique_id', 'title', 'text_content_summary', 'label_mistral_summary', 'explanation_mistral_summary'])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Process news articles for bias detection.")
    parser.add_argument('--data_path', type=str, default='train_llama3.csv', help='Path to the input CSV file containing articles.')
    parser.add_argument('--output_path', type=str, default='Train_mistral-summary_leaderboard.csv', help='Path to save the output CSV file with results.')
    ...
    for i in range(num_batches):
        ...
        if not prompts.empty:
            ...
            save_results(results, output_file=args.output_path)
