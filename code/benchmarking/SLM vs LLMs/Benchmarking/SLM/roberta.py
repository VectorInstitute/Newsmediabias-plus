...
class RobertaClassifier:
    def __init__(self, model_name="FacebookAI/roberta-base", 
                 num_labels=2, max_length=500, dropout_rate=0.2,
                 cache_dir = './cache'):
    ...
    def train(self, train_dataset, train_encoded_labels, batch_size=100, epochs=15, lr=2e-5, patience=5, k_folds=5):
        ...
        for fold, (train_indices, val_indices) in enumerate(kfold.split(train_dataset)):
            ...
            train_dataloader = DataLoader(train_subset, sampler=RandomSampler(train_subset), 
                                          batch_size=batch_size)
            val_dataloader = DataLoader(val_subset, sampler=SequentialSampler(val_subset), 
                                        batch_size=batch_size)
        ...
        plt.ylabel('Loss') 
        plt.legend()
        plt.savefig(f'./{model_name}/loss_plot_5fold.png')
        plt.close()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Bert Classifier for Sequence Classification")
