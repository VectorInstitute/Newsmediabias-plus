...
from transformers import GemmaTokenizer, GemmaForSequenceClassification, AdamW, BitsAndBytesConfig
...
class GemmaClassifier:
    def __init__(self, cache_dir, model_name="google/gemma-2-9b-it", num_labels=2, max_length=500):
        ...
        self.model = GemmaForSequenceClassification.from_pretrained(model_name, 
                                                                    cache_dir=cache_dir,
    ...
    def train(self, train_dataset, train_encoded_labels, 
              batch_size=100, epochs=15, lr=2e-5, patience=5, k_folds=5):
    ...
    def eval(self, dataset, batch_size=100):
        ...
        precision, recall, f1, support = precision_recall_fscore_support(true_vals, 
                                                                         preds, 
                                                                         average='weighted')
        cm = confusion_matrix(true_vals, preds)
        report = classification_report(true_vals, 
                                       preds, 
        ...
        print("\nConfusion Matrix:")
        print(pd.DataFrame(cm, index=self.label_encoder.classes_, 
...
if __name__ == "__main__":
    train_data = pd.read_csv('Benchmarking/sample_train.csv')
    test_data = pd.read_csv('Benchmarking/sample_test.csv')
